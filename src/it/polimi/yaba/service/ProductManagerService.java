...
public class ProductManagerService extends ModelManagerService<Product> {
	...
	public Product create(Map<String, Object> rawData) {
		...
		if (rawData.containsKey("shop")) {
			Shop shop = ShopManagerService.get().select(
					(Key) rawData.get("shop"));
	...
	public boolean exist(String name) {
		Product product = Datastore.query(ProductMeta.get())
				.filter(ProductMeta.get().name.equal(name)).asSingle();
		return product != null;
	}
